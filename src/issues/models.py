...
class AdministrativeRegion(models.Model):
    ...
    administrative_level = models.CharField(max_length=255, db_index=True)
    ...
    def __str__(self):
        if self.parent:
            return f"{self.name} ({self.administrative_level}) - {self.parent.name}"
        return f"{self.name} ({self.administrative_level})"
...
class Issue(models.Model):
    ...
    def __str__(self):
        return (f"{self.status.name} - {self.category.name} - {self.issue_type.name} "
