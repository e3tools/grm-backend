...
class ReporterIssueListAPIViewTest(APITestCase):
    """
    Test cases for the Issue list API endpoint filtered by the authenticated user as reporter.
    ...
    def setUp(self):
        ...
        self.url = reverse("issues:list-reported-issues")
        ...
        # Create issues reported by the test user
    ...
    def test_issue_foreign_key_serialization_structure(self):
        ...
        assert assignee['id'] == self.issue1.assignee.id
        assert assignee['name'] == self.issue1.assignee.name
    ...
    def test_successful_list_retrieval_only_reported(self):
        """Test retrieval only returns issues where the authenticated user is reporter."""
        self.authenticate_with_token()
    ...
    def test_different_users_get_different_results(self):
        """Test that different users only see issues where they are reporter."""
