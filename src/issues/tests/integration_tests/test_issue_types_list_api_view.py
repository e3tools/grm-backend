...
from issues.factories import IssueTypeFactory, UserFactory
...
class TestIssueTypeListAPIView(APITestCase):
    ...
    def setUp(self):
        ...
        self.url = reverse("issues:list-issue-types")

        # Create test user and token
        self.user = UserFactory()
        self.token = Token.objects.create(user=self.user)

        # Create test issue types
        self.info = IssueTypeFactory(name="Info")
        self.complaint = IssueTypeFactory(name="Complaint")
        self.other = IssueTypeFactory(name="Other")
    ...
    def test_successful_list_retrieval_paginated(self):
        """Test successful retrieval of paginated issue types list."""
    ...
    def test_single_type_response(self):
        """Test paginated response when only one type exists."""
        # Delete all but one type
        IssueType.objects.exclude(id=self.info.id).delete()
    ...
    def test_large_dataset_performance(self):
        ...
        IssueTypeFactory.create_batch(50)
    ...
    def test_type_ordering_case_insensitive(self):
        ...
        IssueTypeFactory(name="Apple")
        IssueTypeFactory(name="Banana")
        IssueTypeFactory(name="Cherry")
        IssueTypeFactory(name="Date")
