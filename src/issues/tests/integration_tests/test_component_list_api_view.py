...
from issues.factories import ComponentFactory
from issues.models import Component


@pytest.mark.django_db
@override_settings(LANGUAGE_CODE='en-us')
class ComponentListAPIViewTest(APITestCase):
    """
    Test cases for the Component list API endpoint using Token Authentication.
    ...
    def setUp(self):
        ...
        self.url = reverse("issues:list-components")
    ...
    def test_successful_list_retrieval_paginated(self):
        """Test successful retrieval of paginated components list."""
    ...
    def test_empty_list_when_no_components(self):
        """Test paginated response when no components exist."""
    ...
    def test_single_component_response(self):
        """Test paginated response when only one component exists."""
        # Delete all but one component
        Component.objects.exclude(id=self.component.id).delete()
        ...
        assert response_data['results'][0]['id'] == self.component.id
        assert response_data['results'][0]['name'] == self.component.name
        assert response_data['results'][0]['description'] == self.component.description
    ...
    def test_large_dataset_performance(self):
        """Test paginated response with a larger dataset of components."""
        # Create many more components
        ComponentFactory.create_batch(50)
