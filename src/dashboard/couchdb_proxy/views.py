...
class StatisticsOfTasksUpdatedByRegionView(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if administrative_id:
            stats = eadl_db.get_view_result(
                "tasks", "updated_by_administrative_region_stats", key=administrative_id
            )
        else:
            stats = eadl_db.get_view_result(
                "tasks", "updated_by_administrative_region_stats"
            )
...
class IssuesStatisticsView(
    ...
    def get(self, request, *args, **kwargs):
        ...
        if hasattr(self.request.user, "governmentworker"):
            issues_stats = grm_db.get_view_result(
                "issues", "by_assignee_stats", key=self.request.user.id
            )
        else:
