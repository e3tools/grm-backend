...
class NewIssuePersonFormViewTest(DashboardTestCase):
    def setUp(self):
        super().setUp()
        self.manager = UserFactory(grm_manager=True)
        self.other_user = UserFactory()
        # Issue with required previous step satisfied (contact_medium defaults OK)
        self.issue = IssueFactory(reporter=self.manager, confirmed=False, administrative_region=self.root_region)
        self.url = reverse("dashboard:grm:new_issue_step_2", kwargs={"issue": self.issue.id})
